...
export default async function () {
	...
	const lastCommitText = await Git.run(
		workspace.uri,
		'log',
		'--pretty=format:%H %s',
		'--no-merges',
	...
	const lastCommitList = trimEnd(lastCommitText)
		.split('\n')
		.map((line) => {
	...
	const select = await vscode.window.showQuickPick(lastCommitList, {
		placeHolder: 'Select a commit to squash',
	})
	if (!select) {
		return
	}
	...
	try {
		await vscode.window.withProgress(
			...
			async () => {
				...
				const fileStatusList = trimEnd(fileStatusText)
					.split('\n')
					.map((line) => ({
				...
				const filesHaveBeenPartiallyStaged =
					fileStatusList.some((file) => file.staged) &&
					!fileStatusList.every((file) => file.staged)
				await Git.run(workspace.uri, 'checkout', '--detach', 'HEAD')
				await Git.run(
					workspace.uri,
					'commit',
					'--no-verify',
					filesHaveBeenPartiallyStaged ? '' : '--all',
				...
				await Git.run(
					...
					'--amend',
					'--reset-author',
					'--reuse-message=' + select.commitHash,
				)

				const commitHashText = await Git.run(
					workspace.uri,
					'log',
					'--pretty=format:%H',
					select.commitHash + '..' + originalCommitHash,
				)
				...
				if (status.local) {
					await Git.run(
						workspace.uri,
						'branch',
						status.remote ? `--set-upstream-to=${status.remote}` : '',
						'--force',
						status.local,
					)
