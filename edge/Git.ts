...
import type * as GitBuiltInExtension from './GitBuiltInExtension.d'
import Log from './Log'

export function getGitBuiltInExtension() {
	return vscode.extensions.getExtension<GitBuiltInExtension.GitExtension>(
		'vscode.git',
	)!
}
...
export async function run(
	link: vscode.Uri,
	...formalParameters: Array<string | Options>
): Promise<string> {
	const parameters = formalParameters.filter(
		(parameter): parameter is string =>
			typeof parameter === 'string' && parameter.trim().length > 0,
	)
	const options = formalParameters.find(
		(parameter): parameter is Options =>
...
const runInternal = (
	...
	new Promise<string>((resolve, reject) => {
		...
		const pipe = cp.spawn(gitExecutablePath, actualParameters, {
			cwd: getRepositoryLink(link)?.fsPath.replace(/\\/g, fp.posix.sep),
		})
		if (token) {
...
export async function getPushedCommitHash(link: vscode.Uri) {
	return (
		await run(link, 'log', '--format=%H', '--remotes=origin', '--max-count=1')
	).trim()
}
...
export async function getLocalBranches(link: vscode.Uri) {
	const result = await run(
		...
		'branch',
		'--list',
		'--format=%(objectname) %(refname:short) %(upstream:short) %(authordate:iso-strict)',
	)
...
export async function getRemoteBranches(link: vscode.Uri) {
	const result = await run(
		...
		'--list',
		'--remotes',
		'--format=%(objectname) %(refname:short) %(authordate:iso-strict)',
	)
	return (
		compact(result.split('\n'))
			// Remove "origin"
			.slice(1)
			.map((line) => {
...
export function setRemoteBranch(link: vscode.Uri, localBranchName: string) {
	return run(
		link,
		'branch',
		`--set-upstream-to=origin/${localBranchName}`,
		localBranchName,
	)
}
...
export async function getRemoteHeadBranchName(link: vscode.Uri) {
	...
	return branchName
		.trim()
		.replace(new RegExp('^' + escapeRegExp('refs/remotes/origin/')), '')
}
...
export async function getCurrentBranchStatus(
	link: vscode.Uri,
	...
	if (chunk.includes('...')) {
		...
		const counterpartBranch =
			remoteBranches.find((branch) => branch.name === `origin/${local}`) || ''
...
export async function getFileStatus(link: vscode.Uri) {
	...
	return status
		...
		.map((line) => ({
			...
			currentLink: vscode.Uri.file(
				fp.join(
					repositoryLink.fsPath,
					line.substring(3).split('->').at(-1)!.trim(),
				),
			),
			originalLink: vscode.Uri.file(
				fp.join(
					repositoryLink.fsPath,
...
export async function getBranchTopology(
	link: vscode.Uri,
...
> {
	...
	const commits = chunk(result.trim().split('\n'), 5).map(
		([commit, parentHash, email, date, message]) => {
...
export async function getFileBeforeRenamed(
	link: vscode.Uri,
	...
	const relativeCurrentFilePath = trim(
		link.fsPath.substring(repositoryLink.fsPath.length),
		fp.sep,
	)
	...
	if (
		status &&
		status.symbol === 'R' &&
		status.currentLink.fsPath === link.fsPath
	) {
