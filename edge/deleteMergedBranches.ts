	...
	await vscode.window.withProgress(
		...
		async (progress, token) => {
			...
			for (const workspace of workspaceList) {
				mergedLocalBranches.push(
					...(await getMergedBranchNames(workspace.uri, false)).map((name) => ({
						root: workspace,
						name,
					})),
				)
				mergedRemoteBranches.push(
					...(await getMergedBranchNames(workspace.uri, true)).map((name) => ({
						root: workspace,
						name,
					})),
				)
			...
			if (mergedLocalBranches.length + mergedRemoteBranches.length === 0) {
				vscode.window.showInformationMessage(
					'There were no merged branches to be deleted.',
				)
			...
			const branchNameList = [
				...
				.map(
					([list, unit]) =>
						list.length + ' ' + unit + (list.length > 1 ? 'es' : ''),
				)
				.join(' and ')
			...
			if (mergedRemoteBranches.length === 0) {
				vscode.window.showInformationMessage(
					`${mergedLocalBranches.length} merged local branch${mergedLocalBranches.length > 1 ? 'es have' : ' has'} been deleted.`,
				)
			...
			try {
				for (const branch of mergedRemoteBranches) {
					...
					const branchNameWithoutOrigin = branch.name.substring(
						branch.name.indexOf('/') + 1,
					)
					...
					try {
						...
						if (
							...
							!error.message.includes(
								`error: unable to delete '${branchNameWithoutOrigin}': remote ref does not exist`,
							)
						) {
							throw error
						}
				...
				defer(() => {
					vscode.window.showInformationMessage(
						`${mergedLocalBranches.length + mergedRemoteBranches.length} merged branch${mergedLocalBranches.length + mergedRemoteBranches.length ? 'es have' : 'has'} been deleted.`,
					)
				})
			} catch (error) {
				...
				throw new Error(
					`Deleting merged branches failed - only ${deletedRemoteBranchCount === 1 ? `branch "${mergedRemoteBranches[0].name}" has` : `${deletedRemoteBranchCount} branches have`} been deleted.`,
				)
			}
		},
	)
