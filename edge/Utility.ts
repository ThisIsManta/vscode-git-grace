...
export function getWorkspaceListWithGitEnabled() {
	if (workspaceList === null && vscode.workspace.workspaceFolders) {
		workspaceList = vscode.workspace.workspaceFolders.filter(
			(item) => !!Git.getRepositoryLink(item.uri),
		)
	}
...
export function setWorkspaceAsFirstTryNextTime(
...
export async function updateWorkspaceList(
	...
	workspaceList.push(
		...e.added.filter((item) => !!Git.getRepositoryLink(item.uri)),
	)
...
export async function getCurrentWorkspace(): Promise<vscode.WorkspaceFolder | null> {
	...
	if (workspaceList.length === 0) {
		...
		vscode.window.showErrorMessage(
			'The current folder was not in Git repository.',
			{ modal: true },
		)
	...
	if (vscode.window.activeTextEditor) {
		const currentWorkspace = workspaceList.find(
			(item) =>
				item.uri.fsPath === vscode.window.activeTextEditor?.document.uri.fsPath,
		)
		if (currentWorkspace) {
...
export function getCurrentFile() {
	...
	if (
		Git.getRepositoryLink(vscode.window.activeTextEditor.document.uri) === null
	) {
