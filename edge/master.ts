...
import {
	tryAbortBecauseOfDanglingCommits,
	tryAbortBecauseOfDirtyFiles,
} from './checkout'
	...
	await vscode.window.withProgress(
		...
		async (process, token) => {
			...
			if (status.local === '') {
				...
				const masterHash = await Git.getCurrentCommitHash(
					workspace.uri,
					remoteHeadBranchName,
				)
				if (currentHash === masterHash) {
					vscode.window.showInformationMessage(
						`You are on "${remoteHeadBranchName}" already.`,
					)
				...
				if (
					await tryAbortBecauseOfDanglingCommits(
						workspace.uri,
						`"${remoteHeadBranchName}"`,
					)
				) {
