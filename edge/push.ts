...
export async function pushInternal(workspace: vscode.WorkspaceFolder, options?: {
	...
	try {
		const result = await Git.run(workspace.uri, 'push', status.sync === Git.SyncStatus.LocalIsNotInSyncWithRemote ? '--force-with-lease' : '', 'origin', status.local)
		if (options?.token?.isCancellationRequested) {
			throw new vscode.CancellationError()
		}
		...
		if (error instanceof Git.GitCommandLineError && error.message.includes('error: failed to push some refs')) {
